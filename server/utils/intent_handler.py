...
def id_give_cost(course_id):
    data = database.id_give_cost(course_id)

    resp = "The cost of this course is "
    return resp + str(data[0])


def id_give_title(course_id):
    data = database.id_give_title(course_id)

    resp = "The Title of this course is "
    return resp + str(data[0])


def id_give_credits(course_id):
    data = database.id_give_credits(course_id)

    resp = "The credits attached for this course is "
    return resp + str(data[0])


def id_give_description(course_id):
    data = database.id_give_description(course_id)

    resp = "To explain this course better.. "
    return resp + str(data[0])


def id_give_duration(course_id):
    data = database.id_give_duration(course_id)

    resp = "This course goes on for "
    return resp + str(data[0]) + " days"


def id_give_end(course_id):
    data = database.id_give_end(course_id)

    resp = "The end date of this course is "
    return resp + str(data[0])


def id_give_start(course_id):
    data = database.id_give_start(course_id)

    resp = "This course starts on "
    return resp + str(data[0])


def id_give_subarea(course_id):
    data = database.id_give_subarea(course_id)

    resp = "This course falls under the "
    return resp + str(data[0]) + "Subject Area"


def id_give_tutor(course_id):
    data = database.id_give_tutor(course_id)

    resp = "The tutor of this course is "
    return resp + str(data[0])


def id_give_venue(course_id=None):
    # data = database.id_give_cost(course_id)
...
def credits_give_title(course_credits):
    data = database.credits_give_title(course_credits)
